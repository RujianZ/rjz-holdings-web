    ...
    <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-background/80 border-b border-border">
      <div className="mx-auto max-w-[1200px] px-4 md:px-10 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1 md:gap-2">
          {links.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                className={cn(
                  "mono-label px-1.5 md:px-3 py-2 transition-colors",
                  active
                    ? "text-accent"
          ...
          <Link
            href={altHref}
            className="mono-label px-1.5 md:px-3 py-2 text-muted-foreground hover:text-foreground transition-colors"
